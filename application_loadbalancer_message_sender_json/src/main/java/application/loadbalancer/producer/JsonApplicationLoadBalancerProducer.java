...
public class JsonApplicationLoadBalancerProducer {

	public static void main(String[] args) {
		try {
			JsonApplicationLoadBalancerProducer.restAPISender(args[0], args[1].concat(JsonApplicationLoadBalancerProducer.getTodayDate()),
					Integer.parseInt(args[2]));
		} catch (NumberFormatException e) {
			System.out.println(
					"Pass three parameters: 1 - Application Load Balancer URL (with path), 2 - A string to be used as key for this batch of messages, 3 - Number of Messages in this batch");
			e.printStackTrace();
		}
	}
	...
	public static void restAPISender(String apiGatewayURL, String messageKey, int numberOfMessages) {
		...
		try {
			...
			List<String> people = JsonApplicationLoadBalancerProducer.readDataFile();
			...
			for (int i = 1; i <= numberOfMessagesToSend; i++) {
				Person thisPerson = JsonApplicationLoadBalancerProducer.getPersonFromLine(people.get(i));
				PersonWithID thisPersonWithID = new PersonWithID(thisPerson, messageKey + "-" + i);
				JsonApplicationLoadBalancerProducer.sendRestPostMessage(apiGatewayURL, thisPersonWithID);
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(1);
	...
	public static List<String> readDataFile() {
		...
		InputStream is = JsonApplicationLoadBalancerProducer.class.getClassLoader().getResourceAsStream("us-500.csv");
