...
public class JsonAPIGatewayRESTProducer {
	...
	public static void main(String[] args) {
		try {
			JsonAPIGatewayRESTProducer.restAPISender(args[0], args[1].concat(JsonAPIGatewayRESTProducer.getTodayDate()), Integer.parseInt(args[2]));
		} catch (NumberFormatException e) {
			System.out.println("Pass three parameters: 1 - API URL, 2 - A string to be used as key for this batch of messages, 3 - Number of Messages in this batch");
			e.printStackTrace();
		}
	}
	...
	public static void restAPISender(String apiGatewayURL, String messageKey, int numberOfMessages) {
		...
		try {
			...
			for (int i=1;i<= numberOfMessagesToSend; i++) {
				...
				JsonAPIGatewayRESTProducer.sendRestPostMessage(conn, os, thisPersonWithID);
			}
			os.close();
	...
	public static void sendRestPostMessage(HttpURLConnection conn, OutputStream os, PersonWithID personWithID) {
        try {
