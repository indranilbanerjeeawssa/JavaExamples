...
The application uses several AWS resources, including a Lambda function, an MSK event source and a DynamoDB table to which the lambda function will write. These resources are defined in the `template.yaml` file in this project. You can update the template to add AWS resources through the same deployment process that updates your application code.
...
Use the project ../kafka_senders/kafka_message_sender_json.

Look at the Readme of that project to determine how to build that project and run the command that will send Kafka messages with a Json payload to the lambda function built using this project. The lambda function will receive the Kafka messages with a JSON payload and input fields from the Kafka message into a DynamoDB table.

The value field of each Kafka message that will be sent out will be a Json element of the format
...
Apart from outputting to Cloudwatch logs, the lambda function also inputs fields from the Kafka message (both message metadata fields as well as payload fields from the JSON payload) into a DynamoDB table created by the SAM template. You can log into the AWS console and look at the DynamoDB table and run a scan to see data getting input from the Kafka message into the DynamoDB table
